...
class SecQueFrame(CTk.CTkFrame):
    ...
    def TwoFacAuth(self):
        messagebox.showinfo('2-Factor Authentication',
                            'If you want to enable two factor authentication, just make sure that you have entered the correct email address.\n\nBecause the 2 factor authentication is done by sending an 6 Digit alpha-numeric OTP on the give email address, and the verifying your login through it. \n\nSo, ensure that you have entered the correct and valid email address. \n\nAnd then press the Next for Verifying your email address.')
...
class RegisterPage(CTk.CTk):
    ...
    def addData(self):
        ...
        try:
            ...
            data_to_insert = (first_name, last_name, email, username, password, sec_que, sec_que_ans, sqa_tog, tfa_tog, valid_email)
            insert_query = """
            INSERT INTO "registration" ("first_name", "last_name", "email", "username", "password", "sec_que", "sec_que_ans", "sqa_tog", "tfa_tog", "valid_email")
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
            """            
